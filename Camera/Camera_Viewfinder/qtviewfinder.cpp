    ...
    m_bZoom(true),m_scaleX(1.0),m_scaleY(1.0),m_nBaseWidth(800),m_nBaseHeight(480)
{
    ...
    m_pixmapMenu = QPixmap("/image/menu_icon.png");

    m_returnbtn = new QPushButton(this);
    connect(m_returnbtn, SIGNAL(clicked(bool)), this, SIGNAL(returnbtn_clicked_signal()));
    m_returnbtn->setStyleSheet(QString("QPushButton {border-image: url(:/image/menu_icon.png);}"
                                   "QPushButton:pressed {border-image: url(:/image/menu_icon_pressed.png);}"));

    take_picture = new QPushButton(this);
    connect(take_picture, SIGNAL(clicked(bool)), this, SIGNAL(take_picture_signal()));
    take_picture->setStyleSheet(QString("QPushButton {border-image: url(:/image/take.png);}"
                                   "QPushButton:pressed {border-image: url(:/image/take_pressed.png);}"));
    ...
    show_config->setStyleSheet(QString("QToolButton {border-image: url(:/image/tool.png);}"
                                   "QToolButton:pressed {border-image: url(:/image/tool_pressed.png);}"
                                    "QToolButton:checked {border-image: url(:/image/tool_pressed.png);}"));
